...
/**
 * Created by Nathan on 12/10/2014.
 */
...
public class WaveformSimilarityBasedOverlapAdd {
    ...
    private void applyNewParameters(){
        ...
        Log.d(LOG_TAG, "nominal skip: " + String.valueOf(nominalSkip));
     ...
     * @param position The position where to start the seek operation, in the input buffer.
    ...
    double calcCrossCorr(float[] mixingPos, float[] compare, int offset){
        double corr = 0;
        double norm = 0;
        for (int i = 1; i < overlapLength && i + offset < compare.length; i ++){
    ...
    public float[] process(float[] floatBuffer) {
        ...
        //if (sequenceLength + offset + overlapLength > audioFloatBuffer.length)
        //    sequenceLength = audioFloatBuffer.length - (offset + overlapLength);
        ...
        System.arraycopy(audioFloatBuffer, audioFloatBuffer.length - overlapLength, pMidBuffer, 0, overlapLength);
